...
def test_create_thread_in_view_area_route(test_client):
    ...
    response = test_client.post(f'/area/{area_id}/create_thread', data=new_thread_data, follow_redirects=True)
...
def test_create_message_in_view_thread_route(test_client):
    ...
    response = test_client.post(f'/thread/{thread_id}/send_message', data=new_message_data, follow_redirects=True)
